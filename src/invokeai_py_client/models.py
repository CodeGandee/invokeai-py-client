...
class WorkflowDefinition(BaseModel):
    ...
    id: str | None = Field(None, description="Workflow identifier")
    name: str | None = Field(None, description="Workflow display name")
    description: str | None = Field(None, description="Workflow description")
    version: str = Field("3.0.0", description="Workflow schema version")
    nodes: list[dict[str, Any]] = Field(
        default_factory=list, description="Workflow node definitions"
    )
    edges: list[dict[str, Any]] = Field(
        default_factory=list, description="Node connections"
    )
    form: dict[str, Any] = Field(
        default_factory=dict, description="Public input definitions"
    )
    metadata: dict[str, Any] = Field(
