...
class SmartBoardGenerator:
    def __init__(
        ...
        board_name: str = "auto-test-board-working",
    ):
    ...
    def generate_image(self, prompt_info: Dict[str, str]) -> Optional[str]:
        ...
        before_response = self.session.get(
            f"{self.base_url}/api/v1/images/", params={"limit": 10, "offset": 0}
        )
        before_images = set()
    ...
    def assign_image_to_board(self, image_name: str) -> bool:
        ...
        response = self.session.post(
            ...
            json={"board_id": self.board_id, "image_name": image_name},
        )
        ...
        if response.status_code in [200, 201]:
            ...
            print(
                f"[FAIL] Could not assign {image_name} to board: {response.status_code}"
            )
            return False
