...
def debug_generation():
    ...
    for m in models:
        if (
            m["type"] == "main"
            and m["base"] == "sdxl"
            and "cyberrealistic" in m["name"].lower()
        ):
            model = m
            break
    ...
    response = session.get(
        f"{base_url}/api/v1/images/", params={"limit": 5, "offset": 0}
    )
    ...
    if response.status_code == 200:
        ...
        for i, img in enumerate(images):
            ...
            print(
                f"   Width: {img.get('width', 'N/A')} x Height: {img.get('height', 'N/A')}"
            )
            print(f"   Board: {img.get('board', 'None')}")

            # Try to download
            img_response = session.get(
                f"{base_url}/api/v1/images/i/{img['image_name']}/full"
            )
            if img_response.status_code == 200:
                ...
                if file_size < 50000:
                    print("   [WARNING] Image appears to be black/corrupted")
                else:
                    print("   [OK] Valid image")
    ...
    if images:
        ...
        print(
            f"Method 1 (/full): Status={response.status_code}, Size={len(response.content)} bytes"
        )
