...
def main():
    ...
    with open(test_image_path, "rb") as f:
        ...
        response = session.post(
            f"{base_url}/api/v1/images/upload", files=files, params=params
        )
    ...
    for model in models_data["models"]:
        if model["type"] == "main" and model["base"] == "sdxl":
            sdxl_models.append({"name": model["name"], "key": model["key"]})
    ...
    txt2img_graph = {
        ...
        "nodes": {
            ...
            "neg_prompt": {
                "id": "neg_prompt",
                "type": "sdxl_compel_prompt",
                "inputs": {"prompt": "ugly, blurry, low quality", "style": ""},
            },
            "noise": {
                "id": "noise",
                "type": "noise",
                "inputs": {"width": 1024, "height": 1024, "seed": 12345},
            },
            "denoise": {
                "id": "denoise",
            ...
            "l2i": {"id": "l2i", "type": "l2i", "inputs": {"fp32": False}},
        },
        "edges": [
            {
    ...
    response = session.post(
        f"{base_url}/api/v1/queue/default/enqueue_batch", json=batch_data
    )
    ...
    img2img_graph = {
        ...
        "nodes": {
            ...
            "input_img": {
                "id": "input_img",
                "type": "image",
                "inputs": {"image": {"image_name": uploaded_image_name}},
            },
            "i2l": {"id": "i2l", "type": "i2l", "inputs": {"fp32": False}},
            "pos_prompt": {
                "id": "pos_prompt",
                "type": "sdxl_compel_prompt",
                "inputs": {
            ...
            "noise": {
                "id": "noise",
                "type": "noise",
                "inputs": {"width": 1024, "height": 1024, "seed": 54321},
            },
            "denoise": {
                "id": "denoise",
                "type": "denoise_latents",
    ...
    for i in range(30):  # Wait up to 60 seconds
        ...
        if response.status_code == 200:
            ...
            if queue["in_progress"] == 0 and queue["pending"] == 0:
                ...
                print(
                    f"   ... pending: {queue['pending']}, in progress: {queue['in_progress']}"
                )
